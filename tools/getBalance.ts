...
async function main() {
  ...
  if (ccConfig.enabled) {
    ...
    if (ccBalance.jpy){
      process.stdout.write(`Coinchk, JPY, Cash, ${_.round(ccBalance.jpy)}\n`);
    }
    if (ccBalance.btc){
      process.stdout.write(`Coinchk, BTC, Cash, ${ccBalance.btc}\n`);
    }
    
    // coincheck margin balance
    const ccLeverageBalance = await ccApi.getLeverageBalance();
    process.stdout.write(`Coinchk, JPY, Margin, ${_.round(ccLeverageBalance.margin.jpy)}\n`);
    if(0){
      process.stdout.write(`Coinchk, JPY, Free Margin, ${_.round(ccLeverageBalance.margin_available.jpy)}\n`);
    }
    ...
    if(0){
      process.stdout.write(`Coinchk, BTC, Leverage Position, ${longPosition - shortPosition}\n`);
    }
  ...
  if (quConfig.enabled) {
    ...
    if(0){
      process.stdout.write(`Quoinex, JPY, Margin, ${_.round(quBtcJpyBalance.balance)}\n`);
      process.stdout.write(`Quoinex, JPY, Free Margin, ${_.round(quBtcJpyBalance.free_margin)}\n`);  
      process.stdout.write(`Quoinex, BTC, Leverage Position, ${quBtcJpyBalance.position}\n`);
    }

    // quioine btc rate
    const quPriceLevels = await quApi.getPriceLevels();
    const asks = quPriceLevels.sell_price_levels;
    btcRate = asks[0][0];
